...
class PaypalServices {
  ...
  void initClass() async {
    bool isProduction = (dotenv.env[EnvKeyValue.kModePaypal] ?? "") == "production";
    domain = isProduction ? 'https://api.paypal.com' : 'https://api.sandbox.paypal.com';
    clientId = dotenv.env[EnvKeyValue.kClientIdPaypal] ?? "";
    secret = dotenv.env[EnvKeyValue.kSecretPaypal] ?? "";
    if (!clientId.isValid || !secret.isValid) {
      showToast("Invalid Paypal credentials".tr);
    }
  }
  ...
  Future<Map<String, String>?> createPaypalPayment(transactions, accessToken) async {
    try {
      var response = await post(Uri.parse("$domain/v1/payments/payment"),
